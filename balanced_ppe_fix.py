...
class BalancedPPEFixer:
    def __init__(self, model_path="ppe_quick_finetune/yolov8n_ppe_20epochs/weights/best.pt"):
        """Initialize balanced PPE fixer"""
        self.model_path = model_path
        self.model = YOLO(model_path)
        
        print("="*70)
        print("BALANCED PPE DETECTION FIX")
        print("="*70)
        print(f"[INFO] Model loaded: {model_path}")
        print(f"[INFO] Classes: {self.model.names}")
        print(f"[INFO] Maintaining good helmet detection")
        print(f"[INFO] Fixing goggles, gloves, and safety vests")
    
    def create_balanced_web_app(self):
        """Create balanced web app with proper detection"""
        print("\n[INFO] Creating balanced web application...")
        
        balanced_app = '''#!/usr/bin/env python3
"""
...
'''
        ...
        with open("balanced_ppe_web_app.py", 'w') as f:
            f.write(balanced_app)
        
        print("[SUCCESS] Balanced web application created: balanced_ppe_web_app.py")
...
def main():
    ...
    try:
        # Initialize fixer
        fixer = BalancedPPEFixer()
        
        # Create balanced web app
        fixer.create_balanced_web_app()
        
        print("\n" + "="*70)
        print("BALANCED FIX COMPLETE!")
        print("="*70)
        print(f"\n[SUCCESS] Balanced detection created")
        print(f"[SUCCESS] Maintains good helmet detection")
        print(f"[SUCCESS] Fixes goggles, gloves, and safety vests")
        print(f"[SUCCESS] Different confidence levels per PPE type")
        
        print(f"\n[INFO] To use the balanced fix:")
        print("  1. Stop current web app (Ctrl+C)")
        print("  2. Run: python balanced_ppe_web_app.py")
        print("  3. Test with your construction site image")
        
        print(f"\n[INFO] Balanced fix features:")
        print("  - Helmets: Higher confidence (0.3) - maintains good detection")
        print("  - Goggles: Lower confidence (0.1) + enhancement")
        print("  - Gloves: Lower confidence (0.1) + enhancement")
        print("  - Safety Vests: Very low confidence (0.05) + enhancement")
        print("  - Balanced merging: Different strategies per class")
