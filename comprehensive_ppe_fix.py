#!/usr/bin/env python3
"""
Comprehensive PPE Detection Fix
Addresses all detection issues: goggles, gloves, and accuracy
"""
...
class ComprehensivePPEFixer:
    def __init__(self, model_path="ppe_quick_finetune/yolov8n_ppe_20epochs/weights/best.pt"):
        """Initialize comprehensive PPE fixer"""
        self.model_path = model_path
        self.model = YOLO(model_path)
        
        print("="*70)
        print("COMPREHENSIVE PPE DETECTION FIX")
        print("="*70)
        print(f"[INFO] Model loaded: {model_path}")
        print(f"[INFO] Classes: {self.model.names}")
        print(f"[INFO] Fixing: Goggles, Gloves, Safety Vests, Accuracy")
    
    def create_ultimate_web_app(self):
        """Create the ultimate fixed web application"""
        print("\n[INFO] Creating ultimate fixed web application...")
        
        ultimate_app = '''#!/usr/bin/env python3
"""
Ultimate PPE Detection Web Application
Comprehensive fix for all detection issues
"""
...
def enhance_for_helmets(image):
    """Enhance image specifically for helmets"""
...
def merge_detections_intelligently(all_detections):
    """Intelligently merge detections to avoid duplicates and false positives"""
...
def detect_ppe():
    """Ultimate PPE detection with comprehensive fixes"""
    try:
        ...
        # ULTIMATE DETECTION: Multiple specialized strategies
        all_detections = []
        ...
        # Strategy 2: Enhanced for goggles/glasses
        print("[INFO] Strategy 2: Enhanced for goggles")
        goggles_enhanced = enhance_for_goggles(image)
        results2 = model(goggles_enhanced, conf=0.01, verbose=False)
        for r in results2:
            if r.boxes is not None:
                for box in r.boxes:
                    cls = int(box.cls[0])
        ...
        # Strategy 3: Enhanced for gloves
        print("[INFO] Strategy 3: Enhanced for gloves")
        gloves_enhanced = enhance_for_gloves(image)
        results3 = model(gloves_enhanced, conf=0.01, verbose=False)
        for r in results3:
            if r.boxes is not None:
                for box in r.boxes:
                    cls = int(box.cls[0])
        ...
        # Strategy 5: Enhanced for helmets
        print("[INFO] Strategy 5: Enhanced for helmets")
        helmet_enhanced = enhance_for_helmets(image)
        results5 = model(helmet_enhanced, conf=0.01, verbose=False)
        for r in results5:
            if r.boxes is not None:
                for box in r.boxes:
                    cls = int(box.cls[0])
        ...
        # Intelligently merge all detections
        final_detections = merge_detections_intelligently(all_detections)
...
if __name__ == '__main__':
    print("="*70)
    print("ULTIMATE PPE DETECTION WEB APPLICATION")
    print("="*70)
    print(f"[INFO] Model loaded: {model_path}")
    print(f"[INFO] Classes: {model.names}")
    print(f"[INFO] Comprehensive fixes applied")
    print(f"[INFO] Specialized enhancement for each PPE type")
    print(f"[INFO] Intelligent detection merging")
...
'''
        ...
        with open("ultimate_ppe_web_app.py", 'w') as f:
            f.write(ultimate_app)
        
        print("[SUCCESS] Ultimate web application created: ultimate_ppe_web_app.py")
...
def main():
    ...
    try:
        # Initialize fixer
        fixer = ComprehensivePPEFixer()
        
        # Create ultimate web app
        fixer.create_ultimate_web_app()
        
        print("\n" + "="*70)
        print("COMPREHENSIVE FIX COMPLETE!")
        print("="*70)
        print(f"\n[SUCCESS] Ultimate detection created")
        print(f"[SUCCESS] Specialized enhancement for each PPE type")
        print(f"[SUCCESS] Intelligent detection merging")
        print(f"[SUCCESS] Comprehensive accuracy fixes")
        
        print(f"\n[INFO] To use the ultimate fix:")
        print("  1. Stop current web app (Ctrl+C)")
        print("  2. Run: python ultimate_ppe_web_app.py")
        print("  3. Test with your construction site image")
        
        print(f"\n[INFO] Ultimate fix features:")
        print("  - Goggles: Edge detection + contrast enhancement")
        print("  - Gloves: Yellow/orange color enhancement")
        print("  - Safety Vests: Orange color boost + reflective strips")
        print("  - Helmets: CLAHE contrast enhancement")
        print("  - Intelligent merging to avoid duplicates")
        print("  - Very low confidence (0.01) for maximum sensitivity")
