#!/usr/bin/env python3
"""
Improve Detection Accuracy and Probability
Comprehensive solution to fix all detection errors
"""
...
class DetectionAccuracyImprover:
    def __init__(self, model_path="ppe_quick_finetune/yolov8n_ppe_20epochs/weights/best.pt"):
        """Initialize detection accuracy improver"""
        self.model_path = model_path
        self.model = YOLO(model_path)
        
        print("="*70)
        print("IMPROVING DETECTION ACCURACY AND PROBABILITY")
        print("="*70)
        print(f"[INFO] Model loaded: {model_path}")
        print(f"[INFO] Classes: {self.model.names}")
        print(f"[INFO] Fixing: False positives, Missing detections, Low confidence")
    
    def create_high_accuracy_web_app(self):
        """Create high accuracy web app with improved detection"""
        print("\n[INFO] Creating high accuracy web application...")
        
        high_accuracy_app = '''#!/usr/bin/env python3
"""
...
'''
        ...
        with open("high_accuracy_ppe_web_app.py", 'w') as f:
            f.write(high_accuracy_app)
        
        print("[SUCCESS] High accuracy web application created: high_accuracy_ppe_web_app.py")
...
def main():
    ...
    try:
        # Initialize improver
        improver = DetectionAccuracyImprover()
        
        # Create high accuracy web app
        improver.create_high_accuracy_web_app()
        
        print("\n" + "="*70)
        print("HIGH ACCURACY DETECTION COMPLETE!")
        print("="*70)
        print(f"\n[SUCCESS] High accuracy detection created")
        print(f"[SUCCESS] False positive reduction implemented")
        print(f"[SUCCESS] Specialized enhancement for each PPE type")
        print(f"[SUCCESS] Smart filtering and merging")
        
        print(f"\n[INFO] To use high accuracy detection:")
        print("  1. Stop current web app (Ctrl+C)")
        print("  2. Run: python high_accuracy_ppe_web_app.py")
        print("  3. Test with your construction site image")
        
        print(f"\n[INFO] High accuracy features:")
        print("  - False positive reduction for helmets")
        print("  - Enhanced gloves detection (yellow, orange, grey)")
        print("  - Enhanced safety vest detection (orange, yellow)")
        print("  - Enhanced goggles detection (glasses)")
        print("  - Size and position filtering")
        print("  - Smart confidence thresholds per PPE type")
